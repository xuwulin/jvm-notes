...
对象的内存分配，就是在堆上分配（也可能经过 JIT 编译后被拆散为标量类型并间接在栈上分配），对象主要分配在新生代的 Eden 区上，少数情况下可能直接分配在老年代（大对象），**分配规则不固定**，取决于当前使用的垃圾收集器组合以及相关的参数配置。
...
大对象是指需要大量连续内存空间的 Java 对象，如很长的字符串或数组。

一个大对象能够存入 Eden 区的概率比较小，发生分配担保的概率比较大，而分配担保需要涉及大量的复制，就会造成效率低下。

虚拟机提供了一个 -XX:PretenureSizeThreshold 参数，令大于这个设置值的对象直接在老年代分配，如： -XX:PretenureSizeThreshold = 6M，即当对象大于6M时，直接在老年代分配，这样做的目的是避免在 Eden 区及两个 Survivor 区之间发生大量的内存复制。（还记得吗，新生代采用复制算法回收垃圾）
...
👉 [Previous](https://github.com/xuwulin/jvm/blob/master/docs/04-hotspot-gc.md)
👉 [Next](https://github.com/xuwulin/jvm/blob/master/docs/06-jvm-performance-tuning.md)
👉 [Back to README](https://github.com/xuwulin/jvm/blob/master/README.md)